...
## if [ "$#" -ne 2 ]; then
##   echo "Usage: create_mailchat.sh  "Fecha" "Version""
##   echo "(ie. create_mailchat.sh "20-01-17" "v20.01.001.001.001""  
##   sleep 5 
## exit 1 
## fi  
...
DROPLET_OUTPUT=`curl -X POST "https://api.digitalocean.com/v2/droplets" -H "Content-Type: application/json" -H "Authorization: Bearer $DO_TOKEN" -d '{"name":"'$DROPLET_NAME'","region":"nyc3","size":"'$SIZE'","image":'$IMAGE_ID',"ssh_keys":["45:72:2e:ed:22:2d:ad:d8:0f:57:c0:65:12:e1:8d:61"],"backups":false,"ipv6":false,"user_data":"'"$(cat /home/taskcontrol/user-data-mailchat.yml)"'","private_networking":false,"volumes": null,"tags":["'$DIRECTORIO'"]}'`  
...
## Copiamos nuestra public key al droplet del mailchat server
sshpass -e ssh-copy-id -f ${REMOTE_ROOT_USER}@$IP_ADDRESS < "${PATH_BASE}answeryes.txt" 2>>${PATH_BASE}droplet_mailchat_creation.log 


## Corremos el script de inicializacion del droplet 
sshpass -e ssh ${REMOTE_ROOT_USER}@$IP_ADDRESS 'bash -s' < "${PATH_BASE}bootstrap_malchat.sh" $IP_ADDRESS $DROPLET_NAME 2>>${PATH_BASE}droplet_mailchat_creation.log

current_date_time=`date +%Y-%m-%d-%H:%M:%S`
echo $current_date_time:"----- Droplet listo para su uso: "$IP_ADDRESS >> droplet_mailchat_creation.log
echo " "
echo "---------------------------------------------------------------------------------------"
