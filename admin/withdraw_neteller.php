...
//*******************************************************************************//
//Here is where the Neteller process is configured ********************************//
//*******************************************************************************//
//*******************************************************************************//
//*******************************************************************************//
...
{
	header("location: withdraw_neteller.php");
	exit();
}
...
elseif($Action == 'Edit' && $_POST['Submit']=='Save')
{
	...
	if($_POST['with_status'] == 1)
	{
	   // print_r($_POST);die;
		...
		$pays->InsertWIthdrawInNeteller($_POST);
	}	
	header("location: withdraw_neteller.php?edit=true");
	exit();
}
elseif($Action=='DeleteSelected')
{
	...
	header("location: withdraw_neteller.php?delall=true");
	exit();
}
#=======================================================================================================================================
...
if($Action == 'ViewAll' || $Action == '')
{
	...
	$pays->WithdrawDetails_of_Neteller();
	$rscnt = $db->num_rows();
	...
	$tpl->assign(array( "T_Body"			=>	'withdarw_showall'. $config['tplEx'],
						...
						"A_Action"		    =>	"withdraw_neteller.php",
...
elseif($Action == 'Edit')
{
	...
	$tpl->assign(array("T_Body"					            =>	'withdarw_addedit_neteller'. $config['tplEx'],
						"JavaScript"			=>         array("withdraw_money.js"),
						"Action"				=>	$Action,
