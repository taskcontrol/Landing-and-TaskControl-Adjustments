...
$Action = isset($_GET['Action']) ? $_GET['Action'] : (isset($_POST['Action']) ? $_POST['Action'] : 'ViewAllMercadoPago');
...
{
	header("location: withdraw_mercadopago.php");
	exit();
}
...
elseif($Action == 'Edit' && $_POST['Submit']=='Save')
{
	...
	if($_POST['with_status'] == 1)
	{
	   // print_r($_POST);die;
		...
		$pays->InsertWithdrawInMercadoPago($_POST);
	}	
	header("location: withdraw_mercadopago.php?edit=true");
	exit();
}
elseif($Action=='DeleteSelected')
{
	...
	header("location: withdraw_mercadopago.php?delall=true");
	exit();
}
#=======================================================================================================================================
...
if($Action == 'ViewAllMercadoPago' || $Action == '')
{
	if($_GET['edit']==true)
	...
	$pays->WithdrawDetails_of_Mercadopago();
	$rscnt = $db->num_rows();
	...
	$tpl->assign(array( "T_Body"			=>	'withdarw_showall'. $config['tplEx'],
						...
						"A_Action"		    =>	"withdraw_mercadopago.php",
...
elseif($Action == 'Edit')
{
	...
	$tpl->assign(array("T_Body"					=>	'withdarw_addedit_mercadopago'. $config['tplEx'],
						"JavaScript"		=>  array("withdraw_money.js"),
						"Action"		=>	$Action,
