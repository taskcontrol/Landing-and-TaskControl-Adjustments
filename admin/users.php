...
if(!in_array($Action, array('View', 'Edit', 'Sort', 'View' , 'Send_Mail','MailSelected','Search')))
{
	...
	##	$arr_user_details[$i]['special_start_date'] = $db->f('special_user_start_date');
		$arr_user_details[$i]['membership_buy_date'] = $db->f('membership_buy_date');
		$date_check = explode("-",$db->f('membership_buy_date') );
		$arr_user_details[$i]['membership_finish_date']	= date('Y-m-d', mktime(0,0,0,$date_check[1]+1,($date_check[2]),intval($date_check[0])));
		$i++;
	}
