...
elseif($_POST['Submit1'] == $lang['Button_Submit'])
{
	...
	$earning   =    number_format($importe - $costo_plan,2); 
    ...
    //// Llamo al bash script que crea el droplet nuevo con la compañia, plan id y version
    
    ## Tomo el nombre de la compañia y el pais
    $company_name=$user_details->mem_company;
    $country_id=$user_details->mem_country;
    $company_address=$user_details->mem_address;

    ## Seteamos el nombre de acuerdo al plan solicitado
	
    if ($details->membership_id == 1) $nombre_plan="Starter";
    if ($details->membership_id == 2) $nombre_plan="Professional";
    if ($details->membership_id == 3) $nombre_plan="Enterprise-One";

    ## Seteamos la version actual del aplicativo
    ## $version="v19.03.001.001.001";
    ## $version="v20.03.001.001.0001";
    $version="v20.04.001.001.0001";

    $output=shell_exec('sudo /var/www/taskcontrol.net/web/provisioning/create_droplet.sh "'.$company_name.'" "'.$nombre_plan.'" "'.$version.'" 1 >> /var/www/taskcontrol.net/web/provisioning/droplet_creation.log 2>&1 &'); 
    
    // echo "va a updatear la tabla member_master";
    $member->Update_Member_details($details->membership_id,$transcation_id,$_SESSION['User_Id'],$finished_date,$earning,$ip_address,$version);
    ...
    $mail2->setSubject('TaskControl: Your Subscription Details');
    ...
    // Le enviamos una factura (.pdf) por su subscripcion 
	 ...
	 ## Si pais es distinto de Argentina se le manda la factura Internacional
	 if ( $country_id != 31) {
	     $Tipo="1";
	 }
	 else {  ## Si es de Argentina se le manda recibo luego, formato AFIP (manualmente)
		 ## Si se quiere automatico, hay que integrarlo con el sistema de la AFIP...
    	     $Tipo="2"; 
	 } 
